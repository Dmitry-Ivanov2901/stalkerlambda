...
public class enFire_556 : MonoBehaviour
{
    ...
    void Update()
    {
        if (Time.time - lf >= 0.092f && currentammo >= 1)
        {
            lf = Time.time;
            RaycastHit rc;
            ...
            if (Physics.Raycast(ray, out rc))
            {
                ...
                if (rc.collider.gameObject.GetComponent<playerHealth>() != null && hitChance == 1)
                {
                    ...
                    StartCoroutine(rc.collider.gameObject.GetComponent<playerHealth>().decreaseHealth556());
                }
                if (rc.collider.gameObject.GetComponent<Health>() != null && hitChance == 1 && rc.collider.CompareTag("Friendly"))
                {
                    fire.Play();
                    currentammo--;
